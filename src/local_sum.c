...
int FindLocalSum(image* hIMG, INDEX z, INDEX y, INDEX x)
{
    ...
    if (y > 0 &&  0 < x && x < Nx - 1)
    {
        return SR(hIMG, z, x - 1, y) + SR(hIMG, z, x - 1, y - 1) +
               SR(hIMG, z, x, y - 1) + SR(hIMG, z, x + 1, y - 1);
    }
    else if (y == 0 && x > 0)
    {
        return 4 * SR(hIMG, z, x - 1, y);
    }
    else if (y > 0 && x == 0)
    {
        return 2 * (SR(hIMG, z, x, y - 1) + SR(hIMG, z, x + 1, y - 1));
    }
    else if (y > 0 && x == Nx - 1)
    {
        return SR(hIMG, z, x - 1, y) + SR(hIMG, z, x - 1, y - 1) + 2 * SR(hIMG, z, x, y - 1);
    }
#endif
