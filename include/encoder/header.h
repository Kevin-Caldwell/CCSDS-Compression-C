...
void PrepareImageMetadata(){
    uint8_t essential[SIZEOF_HEADER_ESSENTIAL];
    uint8_t buf;
...
void PreparePredictorMetadata(){
    ...
    // TODO Weight Initialization Resolution
    // 0: Default Weight Initialization Resolution
    Encode(primary, 4, 0, 5);
...
void PrepareSampleAdaptiveEntropyCoder(){
    uint8_t entropy_coder_metadata[2];
